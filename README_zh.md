...
先安装 TAP-Windows6，并创建 friendly name 为 `tap1` 的 TAP 适配器。请从提升权限的管理员终端运行 EtherGuard，接口配置使用 `IType: tap` 与 `Name: tap1`。EtherGuard 是二层 VPN，Wintun 不提供所需的以太网帧接口，不能替代 TAP。

当前依赖基线要求 Go 1.26.5，因此支持 Windows 10 / Windows Server 2016 或更高版本。Windows 7 与 Windows 8 需要旧版 Go 1.20 依赖基线，本分支不再支持。
...
GitHub 托管的 Windows CI 只验证用户态测试与构建；安装 TAP 内核驱动和打开适配器需要独立的提升权限 Windows 测试环境。
